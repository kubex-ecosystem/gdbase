...
type INotificationRule interface {
	...
	GetPlatforms() []NotificationRulePlatform
	SetPlatforms(platforms []NotificationRulePlatform)
	GetJobTypes() []string
	SetJobTypes(jobTypes []string)
	GetUserIDs() []uuid.UUID
	SetUserIDs(userIDs []uuid.UUID)
...
func (n *NotificationRule) SetLastTriggeredAt(lastTriggered *time.Time)        { n.LastTriggeredAt = lastTriggered }
...
func (n *NotificationRule) GetPlatforms() []NotificationRulePlatform {
	if n.Platforms == nil {
		return []NotificationRulePlatform{}
	}

	var platforms []NotificationRulePlatform
	if platformsSlice, ok := n.Platforms.([]interface{}); ok {
		for _, p := range platformsSlice {
			if platform, ok := p.(string); ok {
				platforms = append(platforms, NotificationRulePlatform(platform))
			}
		}
	}
	return platforms
...
func (n *NotificationRule) SetPlatforms(platforms []NotificationRulePlatform) {
	platformStrs := make([]interface{}, len(platforms))
...
func (n *NotificationRule) GetJobTypes() []string {
	if n.JobTypes == nil {
		return []string{}
	}

	var jobTypes []string
	if jobTypesSlice, ok := n.JobTypes.([]interface{}); ok {
		for _, jt := range jobTypesSlice {
			if jobType, ok := jt.(string); ok {
...
func (n *NotificationRule) SetJobTypes(jobTypes []string) {
	jobTypeStrs := make([]interface{}, len(jobTypes))
...
func (n *NotificationRule) GetUserIDs() []uuid.UUID {
	if n.UserIDs == nil {
		return []uuid.UUID{}
	}

	var userIDs []uuid.UUID
	if userIDsSlice, ok := n.UserIDs.([]interface{}); ok {
		for _, uid := range userIDsSlice {
...
func (n *NotificationRule) SetUserIDs(userIDs []uuid.UUID) {
	userIDStrs := make([]interface{}, len(userIDs))
...
func (n *NotificationRule) GetProjectIDs() []uuid.UUID {
	...
	if projectIDsSlice, ok := n.ProjectIDs.([]interface{}); ok {
		for _, pid := range projectIDsSlice {
...
func (n *NotificationRule) SetProjectIDs(projectIDs []uuid.UUID) {
	projectIDStrs := make([]interface{}, len(projectIDs))
	for i, pid := range projectIDs {
		projectIDStrs[i] = pid.String()
	}
...
func (n *NotificationRule) ShouldTriggerForScore(score float64) bool {
	...
	configMap, ok := n.TriggerConfig.(map[string]interface{})
	if !ok {
		return false
	}
